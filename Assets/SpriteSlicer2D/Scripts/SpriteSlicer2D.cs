...
public class SlicedSprite : MonoBehaviour
{
	...
	public void InitFromTK2DSprite(tk2dSprite sprite, ref PolygonCollider2D polygon, ref Vector2[] polygonPoints)
	{
		...
		InitSprite(sprite.gameObject, sprite.GetComponent<MeshRenderer>(), ref polygon, ref polygonPoints, spriteDefinition.uvs[0], spriteDefinition.uvs[spriteDefinition.uvs.Length - 1], spriteDefinition.GetBounds(), spriteDefinition.material, isRotated, isHFlipped, isVFlipped, Vector2.zero);
    ...
    {
        InitSprite(slicedSprite.gameObject, slicedSprite.MeshRenderer, ref polygon, ref polygonPoints, slicedSprite.MinCoords, slicedSprite.MaxCoords, slicedSprite.SpriteBounds, slicedSprite.m_MeshRenderer.sharedMaterial, slicedSprite.Rotated, slicedSprite.HFlipped, slicedSprite.VFlipped, slicedSprite.m_Centroid);
    ...
    public void InitFromUnitySprite(SpriteRenderer unitySprite, ref PolygonCollider2D polygon, ref Vector2[] polygonPoints)
    {
        ...
        InitSprite(unitySprite.gameObject, unitySprite.GetComponent<Renderer>(), ref polygon, ref polygonPoints, minTextureCoords, maxTextureCoords, unitySprite.sprite.bounds, material, false, false, false, Vector2.zero);
    ...
    /// </summary>
    void InitSprite(GameObject parentObject, Renderer parentRenderer, ref PolygonCollider2D polygon, ref Vector2[] polygonPoints, Vector3 minCoords, Vector3 maxCoords, Bounds spriteBounds, Material material, bool rotated, bool hFlipped, bool vFlipped, Vector2 parentCentroid)
    {
        ...
        for (int vertexIndex = 0; vertexIndex < numVertices; vertexIndex++)
        {
            ...
            float widthFraction = 0.5f + (vertex.x * invBoundsSize.x);
            float heightFraction = 0.5f + (vertex.y * invBoundsSize.y);
